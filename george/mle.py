...
class Distribution(ABC):
    ...
    def k(self, nu):
        return np.sqrt(nu)*beta( 1/2,nu/2)
...
class StudentT(Distribution):
    ...
    def student_t_pdf(x,mu,sigma,nu):
        ...
        return (kappa(nu)/sigma) * (1+ ((x-mu)**2)/(nu * sigma**2))**(-(1+nu)/2)
        # return stats.t.pdf(x, nu, loc=mu, scale=sigma)
...
class SkewedStudent(Distribution):
    ...
    def skewed_student_t_pdf(x,mu,sigma,nu,alpha):
        ...
        return kappa(nu)/sigma*(1+1/nu*( (x-mu)/(2*sigma*( alpha*(1-indicator) +(1-alpha)*indicator) ) )**2)**(-(nu+1)/2)
...
#                 kappa(nu_indicator)/sigma*(1+1/nu_indicator*( (x-mu)/(2*sigma*( alpha2*(1-indicator) +(1-alpha2)*indicator) ) )**2)**(-(nu_indicator+1)/2))
